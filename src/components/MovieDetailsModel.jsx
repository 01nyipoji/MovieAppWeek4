...
const MovieDetailsModel = ({movie, onClose, isFavorite, onToggleFavorite}) => {
  ...
  return (<div className='modal modal-open'>
   <div className='modal-box w-full max-w-2xl sm:max-w-3xl bg-base-100 shadow-xl'>
    ...
    <div>
        ...
        <p className='font-semibold'><span>Release Date</span>{' '}{movie.release_date || "N/A"}</p>
        <p className='font-semibold'><span>Rating</span>{movie.vote_average || "N/A"}</p>
        <p className='font-semibold'><span>Geners:</span>{movie.genres ? movie.genres.map(g => g.name).join(', ') : "N/A"}</p>
    </div>
    <div className='flex flex-wrap gap-3 justify-end mt-6'>
    <button className={`btn ${isFavorite ? "btn-error" : "btn-secondary"}`}>{isFavorite ? 'Remove from Favorites' : "Add to Favorites"}</button>
